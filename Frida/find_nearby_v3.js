...
try {
    ...
    // КРИТИЧЕСКАЯ ПРОВЕРКА: Если координаты не прочитались или равны нулю
    if (isNaN(myCurrentX) || isNaN(myCurrentY) || (myCurrentX === 0.0 && myCurrentY === 0.0)) {
        console.log(`[-] Ошибка: Эталонные координаты прочитались как ноль! Текущий адрес ${BASE_COMPONENT_PTR} невалиден. Обнови адрес в скрипте.`);
    } else {
        console.log(`[ЭТАЛОН] Твоя реальная позиция: X = ${myCurrentX.toFixed(6)}, Y = ${myCurrentY.toFixed(6)}`);
        console.log(`[!] Запуск сканирования кучи...`);
        ...
        setTimeout(function() {
            ...
            ranges.forEach(function(range, index) {
                // Оптимизация регионов игровой кучи
                ...
                try {
                    ...
                    for (var offset = 0; offset < size - 0x100; offset += 4) {
                        ...
                        try {
                            // Исключаем пустые структуры, где posX и posY равны нулю
